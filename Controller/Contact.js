const { customerContactResponse, adminContactResponse } = require('../Helpers/EmailsToSend');
...
const postConatctForm = async (req, res) => {
  ...
  try {
    ...
    await customerContactResponse(data, contactForm.email, siteLogo, contactForm);
    await adminContactResponse(data, admin?.userRef?.email, siteLogo, contactForm);

    return res.status(200).json({ message: 'Contact Form Submitted', success: true });
  } catch (e) {
    console.error('Error adding contact', e.message || e);
...
const getContactedUsers = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'successfully Fetched', data: contactForms, success: true });
  } catch (e) {
    console.error('Error getting contacts', e.message || e);
    return res.status(500).json({ message: 'error occuring while fetching Forms', success: false });
  }
};
