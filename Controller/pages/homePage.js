...
module.exports = {
  getHomePageData: async (req, res) => {
    try {
      ...
      for (const sec of sections) {
        if (sec.type === 'feature_collection') {
          sec.content.collections = (sec.content.collections || []).map((id) => collectionsMap[id.toString()]).filter(Boolean);
        }
        if (sec.type === 'feature_product') {
          if (sec.content?.selectedProducts?.length) {
            sec.content.selectedProducts = sec.content.selectedProducts.map((id) => productMap[id.toString()]).filter(Boolean);
          }
          if (sec.content?.selectedcollections?.length) {
            sec.content.selectedcollections = sec.content.selectedcollections.flatMap((cid) => categoryProductMap[cid.toString()] || []);
          }
        }
      }
